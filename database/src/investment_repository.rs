...
impl<'a> Database<'a, Investment> for InvestmentRepository {
    ...
    fn edit(&self, data: Investment) -> Result<(), DatabaseError> {
        let data_json = serde_json::to_string(&data)
            .map_err(|e| DatabaseError::ErrorOnEditData(e.to_string()))?;
        self.db
            .conn
            .execute(
                &format!("UPDATE {} SET data = ?2 WHERE id = ?1", Investment::TITLE),
                params![data.id.to_string(), data_json],
            )
            .map_err(|e| DatabaseError::ErrorOnEditData(e.to_string()))?;
        Ok(())
    ...
    fn suspend(&self, data: Investment) -> Result<(), DatabaseError> {
        self.db
            .conn
            .execute(
                &format!("DELETE FROM {} WHERE id = ?1", Investment::TITLE),
