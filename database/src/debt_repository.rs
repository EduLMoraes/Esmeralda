...
impl<'a> Database<'a, Debt> for DebtRepository {
    fn insert(&self, data: Debt) -> Result<(), DatabaseError> {
        ...
        self.db
            .conn
            .execute(
                &format!("INSERT INTO {} (id, data) VALUES (?1, ?2)", Debt::TITLE),
    ...
    fn edit(&self, data: Debt) -> Result<(), DatabaseError> {
        ...
        self.db
            .conn
            .execute(
                &format!("UPDATE {} SET data = ?2 WHERE id = ?1", Debt::TITLE),
                params![data.id.to_string(), data_json],
            )
            .map_err(|e| DatabaseError::ErrorOnEditData(e.to_string()))?;
        Ok(())
    ...
    fn suspend(&self, data: Debt) -> Result<(), DatabaseError> {
        self.db
            .conn
            .execute(
                &format!("DELETE FROM {} WHERE id = ?1", Debt::TITLE),
    ...
    fn get_data(&self, id: &'a str) -> Result<Debt, DatabaseError> {
        let mut stmt = self
            .db
            .conn
            .prepare(&format!("SELECT data FROM {} WHERE id = ?1", Debt::TITLE))
