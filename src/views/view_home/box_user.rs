...
impl<'a> BoxUser<'a> {
    ...
    pub fn get_box(&self) -> Box {
        ...
        let box_username = CardConfigUser::new("Username: ", Some(&user_instance.username));
        box_username.input.set_editable(false);

        let box_email = CardConfigUser::new("Email: ", Some(&user_instance.email));
        ...
        for (n, people) in peoples_instance.iter().enumerate() {
            ...
            let icon_delete =
                Image::from_file(format!("{}delete.png", env::var("ICON_PATH").unwrap()));
            ...
            let box_name = CardConfigUser::new_with_index(n, "Nome: ", Some(&people.name));
            let box_surname =
                CardConfigUser::new_with_index(n, "Sobrenome: ", Some(&people.surname));
            let box_cell_phone =
                CardConfigUser::new_with_index(n, "Celular: ", Some(&people.cell_phone));

            let box_birthday = CardConfigUser::new_with_index(
                n,
                "Nascimento: ",
                Some(&format!(
                    "{:02}/{:02}/{:04}",
