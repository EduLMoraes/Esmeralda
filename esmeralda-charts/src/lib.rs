...
use thiserror::Error;
use charts_rs::{Box, HorizontalBarChart, LineChart, PieChart, Series, THEME_GRAFANA, CanvasError};
use std::{env, io::{Write, Error as IoError}};
...
impl ChartGeneratorImpl {
    fn process_data(&self, year: i32, debts: &[Debt]) -> (Vec<Series>, Vec<Series>) {
        ...
        for debt in debts.iter().filter(|d| d.date_start.year() == year) {
            ...
            *debtor_summary.entry(debt.debtor.name.clone()).or_insert(0.0) += debt.value;
        }
        
        let monthly_series = monthly_summary.into_iter().map(|(name, data)| (name.as_str(), data).into()).collect();
        let debtor_series = debtor_summary.into_iter().map(|(name, value)| (name.as_str(), vec![value as f32]).into()).collect();
    ...
    fn p_line(&self, year: i32, series: Vec<Series>) -> Result<(), ChartError> {
        let mut plot = LineChart::new_with_theme(
            series,
            vec![
                "Jan".to_string(), "Fev".to_string(), "Mar".to_string(), "Abr".to_string(),
                "Mai".to_string(), "Jun".to_string(), "Jul".to_string(), "Ago".to_string(),
                "Set".to_string(), "Out".to_string(), "Nov".to_string(), "Dez".to_string(),
            ],
            THEME_GRAFANA,
        );
        ...
        plot.legend_margin = Some(Box { top: plot.title_font_size, bottom: 10.0, ..Default::default() });
        plot.y_axis_configs.push(plot.y_axis_configs[0].clone());
        plot.y_axis_configs[0].axis_formatter = Some("R$ {c}".to_string());
        plot.width = 1080.0;
        plot.height = 460.0;

        self.to_save(plot.svg().map_err(|e: CanvasError| ChartError::Generate(e.to_string()))?, "plot_1B")
    }
    ...
    fn p_pie(&self, series: Vec<Series>) -> Result<(), ChartError> {
        ...
        plot.width = 800.0;
        plot.height = 600.0;
        self.to_save(plot.svg().map_err(|e: CanvasError| ChartError::Generate(e.to_string()))?, "plot_1A")
    }

    fn p_bar(&self, series: Vec<Series>) -> Result<(), ChartError> {
        let mut plot = HorizontalBarChart::new_with_theme(series, vec!["Total gasto".to_string()], THEME_GRAFANA);
        plot.width = 800.0;
        plot.height = 600.0;
        self.to_save(plot.svg().map_err(|e: CanvasError| ChartError::Generate(e.to_string()))?, "plot_2A")
    }
    ...
    fn to_save(&self, svg: String, name: &str) -> Result<(), ChartError> {
        ...
        file.write_all(svg.as_bytes()).map_err(|e: IoError| ChartError::Save(e.to_string()))
    }
}